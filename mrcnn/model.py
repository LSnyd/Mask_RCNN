...
ROOT_DIR = os.path.abspath("./model_zoo/Mask_RCNN/")
...
def flip(input, mask, bb, labels, tooth_ids):
    flip_aug = flipper()
    mask = mask.transpose(2, 0, 1)
    sample = flip_aug(image=input.astype(np.uint8), masks=list(mask))
    input = sample['image']
    mask = np.array(sample['masks'])
    mask = mask.transpose(1, 2, 0)

    image_center = (input.shape[0] // 2, input.shape[1] // 2)

    bb[:, [1, 3]] += 2 * (image_center - bb[:, [1, 3]])
    bounding_boxes_w = np.abs(bb[:, 1] - bb[:, 3])
    bb[:, 1] -= bounding_boxes_w
    bb[:, 3] += bounding_boxes_w
    ...
    temp = []
    for label in labels:
        test = tooth_ids[label]
        new_firstdigit = flip_label_dict[str(test)[0]]

        if len(str(test)) == 2:
            new_label = int(new_firstdigit + str(test)[1])

        else:
            new_label = int(new_firstdigit)

        new_idx = tooth_ids.index(str(new_label))
        # new_label = classes.index(str(new_label))
        # one_hot_label = np.zeros(((len(classes)+1)))
        # one_hot_label[1+int(new_label)] = 1
        flipped_labels.append(new_idx)

    return input, mask, bb, np.array(flipped_labels)
...
def load_image_gt(dataset, config, image_id, augmentation=None):
    ...
    image = utils.resize(image, (config.IMAGE_MAX_DIM, config.IMAGE_MAX_DIM, original_shape[2]), preserve_range=True)

    mask_collection = []
    for i in range(mask.shape[2]):
        channel = mask[:, :, i]
        mask_collection.append(utils.resize(channel.astype(bool), (config.IMAGE_MAX_DIM, config.IMAGE_MAX_DIM)))

    mask = np.moveaxis(mask_collection, 0, 2)
    mask = np.round(mask).astype(bool)

    window = [config.IMAGE_MAX_DIM, config.IMAGE_MAX_DIM, 0, 0]
    scale = 1
    ...
    if augmentation is not None:
        ...
        sample = augmentation(image=image.astype(np.uint8), masks=list(mask))
        image = sample["image"]

        mask = np.array(sample['masks'])
        mask = mask.transpose(1, 2, 0)

        # Flip labels and bboxes if mask flipped
        if np.array_equal(mask, unaug_mask) is False:
            bbox = utils.flip_bboxes(bbox, image)
            class_ids = utils.flip_tooth_label(class_ids, dataset.return_labels())
...
class DataGenerator(KU.Sequence):
    ...
    def __init__(self, dataset, config, shuffle=True, augmentation=None, augmentation_p=0.5, flipping=False,
                 flipping_p=0.5,
...
class MaskRCNN(object):
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
              augmentation=None, augmentation_p=0.5, flipping=False, flipping_p=0.5, custom_callbacks=None,
        ...
        train_generator = DataGenerator(train_dataset, self.config, shuffle=True,
                                        augmentation=augmentation, augmentation_p=augmentation_p, flipping=flipping,
                                        flipping_p=flipping_p)
...
class DataGenerator(KU.Sequence):
    ...
    def __init__(self, dataset, config, shuffle=True, augmentation=None, augmentation_p=0.5, flipping=False, flipping_p=0.5,
                 random_rois=0, detection_targets=False):
...
class MaskRCNN(object):
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
              augmentation=None, augmentation_p=0.5, flipping=False, flipping_p=0.5,custom_callbacks=None, no_augmentation_sources=None):
        """Train the model.
        train_dataset, val_dataset: Training and validation Dataset objects.
        learning_rate: The learning rate to train with
        epochs: Number of training epochs. Note that previous training epochs
        ...
        # Data generators
        train_generator = DataGenerator(train_dataset, self.config, shuffle=True,
                                         augmentation=augmentation, augmentation_p=augmentation_p, flipping=flipping, flipping_p=flipping_p)
        val_generator = DataGenerator(val_dataset, self.config, shuffle=True)
